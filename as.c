...
static Boolean ReadMacro_SearchSect(char *Test_O, char *Comp, Boolean *Erg, LongInt *Section)
{
  ...
  strmaxcpy(Test, Test_O, STRINGSIZE);
  KillBlanks(Test);
  p = strchr(Test, ':');
  if (!p)
    *Sect = '\0';
...
static void ExpandMacro(PMacroRec OneMacro)
{
  ...
  if ((NestMax > 0) && (OneMacro->UseCounter > NestMax))
    WrError(ErrNum_RekMacro);
  else
  {
    OneMacro->UseCounter++;
...
static void ExpandEXITM(void)
{
  ...
  if (!ChkArgCnt(0, 0))
    ...
    WrError(ErrNum_EXITMOutsideMacro);
  else if (!FirstInputTag->IsMacro)
    WrError(ErrNum_EXITMOutsideMacro);
  else if (IfAsm)
  {
...
void WriteCode(void)
{
  ...
  else
  {
    ...
    if (ActPC == StructSeg)
    {
      if ((CodeLen != 0) && (!DontPrint))
        WrError(ErrNum_NotInStruct);
...
static void AssembleFile_InitPass(void)
{
  ...
  ResetMacroDefines();
  ResetStructDefines();
  StrCompMkTemp(&TmpComp, FlagTrueName);
  EnterIntSymbol(&TmpComp, 1, 0, True);
  StrCompMkTemp(&TmpComp, FlagFalseName);
  EnterIntSymbol(&TmpComp, 0, 0, True);
  StrCompMkTemp(&TmpComp, PiName);
  EnterFloatSymbol(&TmpComp, 4.0 * atan(1.0), True);
  StrCompMkTemp(&TmpComp, VerName);
  EnterIntSymbol(&TmpComp, VerNo, 0, True);
  as_snprintf(ArchVal, sizeof(ArchVal), "%s-%s", ARCHPRNAME, ARCHSYSNAME);
  StrCompMkTemp(&TmpComp, ArchName);
  EnterStringSymbol(&TmpComp, ArchVal, True);
  ...
  EnterIntSymbol(&TmpComp, 0, 0, True);
#endif
  StrCompMkTemp(&TmpComp, CaseSensName);
  EnterIntSymbol(&TmpComp, Ord(CaseSensitive), 0, True);
  if (PassNo == 0)
  {
  ...
  StrCompMkTemp(&TmpComp, ListOnName);
  EnterIntSymbol(&TmpComp, ListOn = 1, SegNone, True);
  ...
  StrCompMkTemp(&TmpComp, NestMaxName);
  EnterIntSymbol(&TmpComp, NestMax = DEF_NESTMAX, SegNone, True);
  CopyDefSymbols();
